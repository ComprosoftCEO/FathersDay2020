...
fn talk_mayor(state: &mut State) -> GameAction {
  if !state.has_or_used_item(Item::OldKey) {
    ...
    GameAction::ShowMessage(MessageType::Generic("Welcome to our small village. I hear you are trying to get into the fortress. It probably won't help much, but I've saved the old key used to unlock the gate. Unfortunately, the bridge is gone, so you will need to find some other way to cross the river.\n\nThe mayor gave you an 'oldkey'.".into()))
  } else {
    GameAction::ShowMessage(MessageType::Generic("Good luck on your quest!".into()))
  }
}
